...
function SelectboxComponent({ options = [], label = 'Seçenek', defaultValue, icon, onChange, className }) {
  ...
  return (
    <div ref={rootRef} className={`relative group flex items-center gap-x-[16px] select-none w-1/3 ${className ?? ''}`}>
      <button
        ref={buttonRef}
        type='button'
        ...
        className='flex items-center gap-x-[16px] cursor-pointer w-full text-left outline-none'
      >
        ...
        <div className='flex flex-1 items-center justify-between'>
          ...
          <LuChevronDown className={`group-hover:text-primary transition-all duration-300 ease-in-out text-slate-500 ${isOpen ? 'rotate-180' : ''}`} />
        </div>
      </button>
      ...
      {isOpen && (
        <ul
          ...
          className='absolute left-0 top-[calc(100%+8px)] z-20 w-[280px] rounded-[12px] border border-slate-200 bg-white p-[6px] shadow-lg'
        >
